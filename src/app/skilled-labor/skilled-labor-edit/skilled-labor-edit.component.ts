...
@Component({
  selector: "app-skilled-labor-edit",
  templateUrl: "./skilled-labor-edit.component.html",
...
export class SkilledLaborEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem("skilledLaborPage") != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem("skilledLaborPage"));

      for (let index = 0; index < model.length; index++) {
        if (index == 0) this.permission = model[index].title;
    ...
    this.activetedRoute.queryParams.subscribe((params) => {
      ...
      if (this.code != "") {
        this.title = "แก้ไขข้อมุลฝีมือแรงงาน";
        this.read();
        this.readSkilledLaborRegister();
      }
    });
  ...
  create() {
    ...
    if (this.editModel.title == "") {
      this.toastr.warning("กรุณาใส่สาขา", "แจ้งเตือนระบบ", { timeOut: 2000 });
      isValid = true;
    }
    ...
    this.serviceProviderService
      .post("skilledLabor/create", this.editModel)
      .subscribe(
        (data) => {
          let model: any = {};
          model = data;
  ...
  readSkilledLaborRegister() {
    this.criteriaModel.code = this.code;
    this.serviceProviderService
      .post("skilledLabor/register/read", this.criteriaModel)
      .subscribe(
        (data) => {
          let model: any = {};
          model = data;
          ...
          this.paginationModel.itemsPerPageString =
            this.paginationModel.itemsPerPage.toString();

          if (
            this.criteriaModel.skip + this.paginationModel.itemsPerPage >
            this.paginationModel.totalItems
          )
            this.paginationModel.textPage =
              this.paginationModel.totalItems != 0
                ? "แสดง " +
                  (this.criteriaModel.skip + 1) +
                  " ถึง " +
                  this.paginationModel.totalItems +
                  " จาก " +
                  this.paginationModel.totalItems +
  ...
  update() {
    ...
    this.serviceProviderService
      .post("skilledLabor/update", this.editModel)
      .subscribe(
        (data) => {
          this.serviceProviderService
  ...
  readCategory(param) {
    ...
    if (this.editModel.language != "") {
      this.serviceProviderService
        .post("skilledLabor/category/read", {
          language: param,
  ...
  galleryRead() {
    this.serviceProviderService
      .post("m/skilledLabor/gallery/read", { code: this.editModel.code })
      .subscribe(
        (data) => {
          let model: any = {};
          model = data;
  ...
  checkSave() {
    ...
    if (this.editModel.category == "") {
      this.toastr.warning("กรุณาเลือกหมวดหมู", "แจ้งเตือนระบบ", {
        timeOut: 1000,
      });
      isValid = true;
    }
  ...
  checkOG(param, param2) {
    ...
    this.organization.forEach((og) => {
      if (og.lv4 != "") {
        ...
        this.serviceProviderService
          .post("organization/read", { category: "lv4" })
          .subscribe(
            (data) => {
              let model: any = {};
              model = data;
              ...
              split.forEach((element) => {
                let idx = model.objectData.findIndex(
                  (item) => item.code === element,
                );
                if (idx != -1) {
                  let display = "";
                  if (this.lv3Category.length > 0) {
                    let idxy = this.lv3Category.findIndex(
                      (item) => item.code === model.objectData[idx].lv0,
                    );
                    if (idxy != -1) display = this.lv3Category[idxy] + " - ";
                  }

                  model.objectData[idx].display =
                    display + model.objectData[idx].title;

                  let idxx = this.lv4Category.findIndex(
                    (item) => item.code === model.objectData[idx].code,
                  );
                  if (idxx == -1) this.lv4Category.push(model.objectData[idx]);
              ...
              setTimeout(() => {
                if (this.isReadOGLv4) {
                  ...
                  lvContent.forEach((element) => {
                    let idx = this.lv4Category.findIndex(
                      (item) => item.code === element,
                    );
                    if (idx != -1) {
        ...
        this.serviceProviderService
          .post("organization/read", { category: "lv3" })
          .subscribe(
            (data) => {
              let model: any = {};
              model = data;
              ...
              split.forEach((element) => {
                ...
                if (idx != -1) {
                  ...
                  if (this.lv2Category.length > 0) {
                    let idxy = this.lv2Category.findIndex(
                      (item) => item.code === model.objectData[idx].lv0,
                    );
                    if (idxy != -1) display = this.lv2Category[idxy] + " - ";
                  }

                  model.objectData[idx].display =
                    display + model.objectData[idx].title;

                  let idxx = this.lv3Category.findIndex(
                    (item) => item.code === model.objectData[idx].code,
                  );
                  if (idxx == -1) this.lv3Category.push(model.objectData[idx]);
                  // arr.push({ title: model.objectData[idx].title, display: model.objectData[idx].title, code: model.objectData[idx].code, check: false });
                }
              });
              ...
              setTimeout(() => {
                if (this.isReadOGLv3) {
                  ...
                  lvContent.forEach((element) => {
                    let idx = this.lv3Category.findIndex(
                      (item) => item.code === element,
                    );
                    if (idx != -1) {
        ...
        this.serviceProviderService
          .post("organization/read", { category: "lv2" })
          .subscribe(
            (data) => {
              let model: any = {};
              model = data;
              ...
              split.forEach((element) => {
                ...
                if (idx != -1) {
                  ...
                  if (this.lv1Category.length > 0) {
                    let idxy = this.lv1Category.findIndex(
                      (item) => item.code === model.objectData[idx].lv0,
                    );
                    if (idxy != -1) display = this.lv1Category[idxy] + " - ";
                  }

                  model.objectData[idx].display =
                    display + model.objectData[idx].title;

                  let idxx = this.lv2Category.findIndex(
                    (item) => item.code === model.objectData[idx].code,
                  );
                  if (idxx == -1) this.lv2Category.push(model.objectData[idx]);
                  // arr.push({ title: model.objectData[idx].title, display: model.objectData[idx].title, code: model.objectData[idx].code, check: false });
                }
              });
              ...
              setTimeout(() => {
                if (this.isReadOGLv2) {
                  ...
                  lvContent.forEach((element) => {
                    let idx = this.lv2Category.findIndex(
                      (item) => item.code === element,
                    );
                    if (idx != -1) {
        ...
        this.serviceProviderService
          .post("organization/read", { category: "lv1" })
          .subscribe(
            (data) => {
              let model: any = {};
              model = data;
              ...
              split.forEach((element) => {
                ...
                if (idx != -1) {
                  ...
                  if (this.lv0Category.length > 0) {
                    let idxy = this.lv0Category.findIndex(
                      (item) => item.code === model.objectData[idx].lv0,
                    );
                    if (idxy != -1) display = this.lv0Category[idxy] + " - ";
                  }

                  model.objectData[idx].display =
                    display + model.objectData[idx].title;

                  let idxx = this.lv1Category.findIndex(
                    (item) => item.code === model.objectData[idx].code,
                  );
                  if (idxx == -1) this.lv1Category.push(model.objectData[idx]);
                  // arr.push({ title: model.objectData[idx].title, display: model.objectData[idx].title, code: model.objectData[idx].code, check: false });
                }
              });
              ...
              setTimeout(() => {
                if (this.isReadOGLv1) {
                  ...
                  lvContent.forEach((element) => {
                    let idx = this.lv1Category.findIndex(
                      (item) => item.code === element,
                    );
                    if (idx != -1) {
        ...
        this.serviceProviderService
          .post("organization/read", { category: "lv0" })
          .subscribe(
            (data) => {
              let model: any = {};
              model = data;
              ...
              split.forEach((element) => {
                ...
                if (idx != -1) {
                  ...
                  let idxx = this.lv0Category.findIndex(
                    (item) => item.code === model.objectData[idx].code,
                  );
                  if (idxx == -1) this.lv0Category.push(model.objectData[idx]);
                  // arr.push({ title: model.objectData[idx].title, display: model.objectData[idx].title, code: model.objectData[idx].code, check: false });
                }
              });
              ...
              setTimeout(() => {
                if (this.isReadOGLv0) {
                  ...
                  lvContent.forEach((element) => {
                    let idx = this.lv0Category.findIndex(
                      (item) => item.code === element,
                    );
                    if (idx != -1) {
  ...
  paginationModelChanged(changes: KeyValueChanges<string, any>) {
    ...
    this.criteriaModel.skip =
      this.paginationModel.currentPage == 1
        ? 0
        : this.paginationModel.currentPage * this.paginationModel.itemsPerPage -
          this.paginationModel.itemsPerPage; // <----- Pagination
