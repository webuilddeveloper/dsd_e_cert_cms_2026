...
@Component({
  selector: 'app-skilled-labor-criteria',
  templateUrl: './skilled-labor-criteria.component.html',
  styleUrls: ['./skilled-labor-criteria.component.css']
})
export class SkilledLaborCriteriaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('skilledLaborPage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('skilled-laborPage'));
  ...
  readSequence() {
    ...
    this.serviceProviderService.post('groupby/read', { permission: this.permission,title: 'skilledLabor' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  readCategory() {
    ...
    this.serviceProviderService.post('skilledLabor/category/read', this.criteriaModel).subscribe(data => {
      let model: any = {};
      model = data;
