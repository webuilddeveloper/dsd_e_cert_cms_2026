...
@Component({
  selector: 'app-skilled-labor-list',
  templateUrl: './skilled-labor-list.component.html',
  styleUrls: ['./skilled-labor-list.component.css']
})
export class SkilledLaborListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('skilledLaborPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('skilledLaborPage'));
    }
  ...
  create() {
    this.router.navigate(['skilled-labor-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['skilled-labor-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('skilledLabor/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('skilledLabor/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
