...
@Component({
  selector: 'app-skilled-labor-category-list',
  templateUrl: './skilled-labor-category-list.component.html',
  styleUrls: ['./skilled-labor-category-list.component.css']
})
export class SkilledLaborCategoryListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('skilledLaborCategoryPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('skilledLaborCategoryPage'));
    }
  }
  ...
  create() {
    this.router.navigate(['skilled-labor-category-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['skilled-labor-category-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('skilledLabor/category/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('skilledLabor/category/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
