...
@Component({
  selector: 'app-skilled-labor-category-edit',
  templateUrl: './skilled-labor-category-edit.component.html',
  styleUrls: ['./skilled-labor-category-edit.component.css']
})
export class SkilledLaborCategoryEditComponent implements OnInit {

  title = 'เพิ่มข้อมูลหมวดหมู่ฝีมือแรงงาน';
  editModel: any = {};
  code: any;
  category: any;
  ...
  ngOnInit(): void {
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.read();
        this.title = 'แก้ไขข้อมูลหมวดหมู่ฝีมือแรงงาน';
      }
  ...
  create() {
    ...
    this.serviceProviderService.post('skilledLabor/category/create', this.editModel).subscribe(data => {
      let model: any = {}
      model = data;
      if (model.status === 'S') {
  ...
  read() {
    this.serviceProviderService.post('skilledLabor/category/read', { code: this.code, permission: 'all' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    if (this.editModel.image != undefined)
      this.editModel.imageUrl = this.editModel.image[0].imageUrl;
    this.serviceProviderService.post('skilledLabor/category/update', this.editModel).subscribe(data => {
      let model: any = {}
      model = data;
      if (model.status === 'S') {
        /** spinner ends after 5 seconds */
  ...
  back() {
    this.router.navigate(['skilled-labor-category'], { skipLocationChange: true });
  }
