...
export class UserRoleEditComponent implements OnInit {
            ...
            .subscribe(
              (data) => {
                ...
                this.messageInput.forEach((element) => {
                  ...
                  if (element.skilledLaborPage)
                    element.title = element.skilledLaborCategoryList[0].title;
                });
  ...
  checkAllCategory() {
    ...
    this.editModel.skilledLaborCategoryPage = true;
  ...
  checkAllMain() {
    ...
    this.editModel.skilledLaborPage = true;
  ...
  readPoiCategory() {
    this.serviceProviderService.post("m/poi/category/read", {}).subscribe(
      (data) => {
        let model: any = {};
        model = data;
        console.log("model", model);
        model.objectData.forEach((element) => {
          this.poiCategory.push({
            value: element.code,
            display: element.title,
          });
        });
  ...
  readNotificationCategory() {
    this.serviceProviderService
      .post("m/notification/category/read", {})
      .subscribe(
        (data) => {
          let model: any = {};
          model = data;
          console.log("model", model);
          model.objectData.forEach((element) => {
            this.notificationCategory.push({
              value: element.code,
              display: element.title,
            });
          });
  ...
  readWelfareCategory() {
    this.serviceProviderService.post("m/welfare/category/read", {}).subscribe(
      (data) => {
        let model: any = {};
        model = data;
        console.log("welfare", model);
        model.objectData.forEach((element) => {
          this.welfareCategory.push({
            value: element.code,
            display: element.title,
          });
        });
  ...
  readTrainingCategory() {
    this.serviceProviderService.post("m/training/category/read", {}).subscribe(
      (data) => {
        let model: any = {};
        model = data;
        console.log("training", model);
        model.objectData.forEach((element) => {
          this.trainingCategory.push({
            value: element.code,
            display: element.title,
          });
        });
  ...
  readSkilledLaborCategory() {
    this.serviceProviderService
      .post("m/skilledLabor/category/read", {})
      .subscribe(
        (data) => {
          let model: any = {};
          model = data;
          console.log("training", model);
          model.objectData.forEach((element) => {
            this.skilledLaborCategory.push({
              value: element.code,
              display: element.title,
            });
          });
  ...
  readWarningCategory() {
    this.serviceProviderService.post("m/warning/category/read", {}).subscribe(
      (data) => {
        let model: any = {};
        model = data;
        console.log("warning", model);
        model.objectData.forEach((element) => {
          this.warningCategory.push({
            value: element.code,
            display: element.title,
          });
        });
  ...
  readFundCategory() {
    this.serviceProviderService.post("m/fund/category/read", {}).subscribe(
      (data) => {
        let model: any = {};
        model = data;
        console.log("fund", model);
        model.objectData.forEach((element) => {
          this.fundCategory.push({
            value: element.code,
            display: element.title,
          });
        });
  ...
  chooseAllEvertThing() {
    ...
    if (this.editModel.warningPage) this.chooseAllWarning();

    if (this.editModel.reporterPage) this.chooseAllReporter();

    if (this.editModel.trainingPage) this.chooseAllTraining();

    if (this.editModel.skilledLaborPage) this.chooseAllSkilledLabor();

    if (this.editModel.notificationPage) this.chooseAllWelfare();

    if (this.editModel.notificationPage) this.chooseAllNotification();

    if (this.editModel.suggestionPage) this.chooseAllSuggestion();

    if (this.editModel.pollPage) this.chooseAllPoll();

    if (this.editModel.poiPage) this.chooseAllPoi();
  ...
  async chooseAllContact() {
    await this.contactCategory.forEach((element) => {
      if (
        this.messageInput.findIndex(
          (item) =>
            item.category == element.value && item.page == "เบอร์ติดต่อ",
        ) === -1
      ) {
        this.messageInput.splice(0, 0, {
          page: "เบอร์ติดต่อ",
  ...
  async chooseAllKnowledge() {
    await this.knowledgeCategory.forEach((element) => {
      if (
        this.messageInput.findIndex(
          (item) =>
            item.category == element.value && item.page == "คลังความรู้",
        ) === -1
      ) {
        this.messageInput.splice(0, 0, {
          page: "คลังความรู้",
  ...
  async chooseAllPoi() {
    await this.poiCategory.forEach((element) => {
      if (
        this.messageInput.findIndex(
          (item) =>
            item.category == element.value && item.page == "สถานที่น่าสนใจ",
        ) === -1
      ) {
        this.messageInput.splice(0, 0, {
          page: "สถานที่น่าสนใจ",
  ...
  async chooseAllSuggestion() {
    await this.suggestionCategory.forEach((element) => {
      if (
        this.messageInput.findIndex(
          (item) => item.category == element.value && item.page == "ข้อเสนอแนะ",
        ) === -1
      ) {
        this.messageInput.splice(0, 0, {
          page: "ข้อเสนอแนะ",
  ...
  async chooseAllNotification() {
    await this.notificationCategory.forEach((element) => {
      if (
        this.messageInput.findIndex(
          (item) => item.category == element.value && item.page == "แจ้งเตือน",
        ) === -1
      ) {
        this.messageInput.splice(0, 0, {
          page: "แจ้งเตือน",
  ...
  async chooseAllWelfare() {
    await this.welfareCategory.forEach((element) => {
      if (
        this.messageInput.findIndex(
          (item) => item.category == element.value && item.page == "สวัสดิการ",
        ) === -1
      ) {
        this.messageInput.splice(0, 0, {
          page: "สวัสดิการ",
  ...
  async chooseAllTraining() {
    await this.trainingCategory.forEach((element) => {
      if (
        this.messageInput.findIndex(
          (item) => item.category == element.value && item.page == "ฝึกอบรม",
        ) === -1
      ) {
        this.messageInput.splice(0, 0, {
          page: "ฝึกอบรม",
  ...
  async chooseAllSkilledLabor() {
    await this.skilledLaborCategory.forEach((element) => {
      if (
        this.messageInput.findIndex(
          (item) =>
            item.category == element.value && item.page == "ฝีมือแรงงาน",
        ) === -1
      ) {
        this.messageInput.splice(0, 0, {
          page: "ฝีมือแรงงาน",
  ...
  async chooseAllWarning() {
    await this.warningCategory.forEach((element) => {
      if (
        this.messageInput.findIndex(
          (item) =>
            item.category == element.value && item.page == "ข่าวเตือนภัย",
        ) === -1
      ) {
        this.messageInput.splice(0, 0, {
          page: "ข่าวเตือนภัย",
  ...
  async chooseAllFund() {
    await this.fundCategory.forEach((element) => {
      if (
        this.messageInput.findIndex(
          (item) => item.category == element.value && item.page == "กองทุน",
        ) === -1
      ) {
        this.messageInput.splice(0, 0, {
          page: "กองทุน",
  ...
  async chooseAllCooperativeForm() {
    await this.cooperativeFormCategory.forEach((element) => {
      if (
        this.messageInput.findIndex(
          (item) =>
            item.category == element.value && item.page == "แบบฟอร์มตำรวจ",
        ) === -1
      ) {
        this.messageInput.splice(0, 0, {
          page: "แบบฟอร์มตำรวจ",
    ...
    if (category != "") {
      if (
        ...
        if (param == "newsPage") {
          ...
          if (
            this.poiCategory.findIndex((item) => item.value == category) != -1
          ) {
            let idx = this.poiCategory.findIndex(
              (item) => item.value == category,
            );
            this.messageInput.splice(0, 0, {
              page: page,
              category: category,
              title: this.poiCategory[idx].display,
              poiPage: true,
            });
          }
          ...
          if (
            this.suggestionCategory.findIndex(
              (item) => item.value == category,
            ) != -1
          ) {
          ...
          if (
            this.notificationCategory.findIndex(
              (item) => item.value == category,
            ) != -1
          ) {
            let idx = this.notificationCategory.findIndex(
              (item) => item.value == category,
            );
            this.messageInput.splice(0, 0, {
              page: page,
              category: category,
              title: this.notificationCategory[idx].display,
              notificationPage: true,
            });
          }
        } else if (param == "welfarePage") {
          if (
            this.welfareCategory.findIndex((item) => item.value == category) !=
            -1
          ) {
            let idx = this.welfareCategory.findIndex(
              (item) => item.value == category,
            );
            this.messageInput.splice(0, 0, {
              page: page,
              category: category,
              title: this.welfareCategory[idx].display,
              welfarePage: true,
            });
          }
        } else if (param == "trainingPage") {
          if (
            this.trainingCategory.findIndex((item) => item.value == category) !=
            -1
          ) {
            let idx = this.trainingCategory.findIndex(
              (item) => item.value == category,
            );
            this.messageInput.splice(0, 0, {
              page: page,
              category: category,
              title: this.trainingCategory[idx].display,
              trainingPage: true,
            });
          }
        } else if (param == "skilledLaborPage") {
          if (
            this.skilledLaborCategory.findIndex(
              (item) => item.value == category,
            ) != -1
          ) {
            let idx = this.skilledLaborCategory.findIndex(
              (item) => item.value == category,
            );
            this.messageInput.splice(0, 0, {
              page: page,
              category: category,
              title: this.skilledLaborCategory[idx].display,
              skilledLaborPage: true,
            });
          }
          ...
          if (
            this.warningCategory.findIndex((item) => item.value == category) !=
            -1
          ) {
            let idx = this.warningCategory.findIndex(
              (item) => item.value == category,
            );
            this.messageInput.splice(0, 0, {
              page: page,
              category: category,
              title: this.warningCategory[idx].display,
              warningPage: true,
            });
          }
        } else if (param == "fundPage") {
          if (
            this.fundCategory.findIndex((item) => item.value == category) != -1
          ) {
            let idx = this.fundCategory.findIndex(
              (item) => item.value == category,
            );
            this.messageInput.splice(0, 0, {
              page: page,
              category: category,
              title: this.fundCategory[idx].display,
              fundPage: true,
            });
          }
